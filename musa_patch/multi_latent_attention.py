...
class MLASelfAttention(MultiLatentAttention):
    ...
    def get_query_key_value_tensors(
        ...
        if self.config.q_lora_rank is not None:
            ...
            def custom_forward(input_q):
                q_compressed = self.q_layernorm(input_q)
                q, _ = self.linear_q_up_proj(q_compressed)
            
                return q, _

            if self.config.q_rms_recompute == True:
                # print("use selective_rms_q ")
                q, _ = tensor_parallel.checkpoint(custom_forward, False, q_compressed)
            else:
                # q_compressed = self.q_layernorm(q_compressed)
                # q, _ = self.linear_q_up_proj(q_compressed)
                q, _ = custom_forward(q_compressed)
        else:
        ...
        # kv: [s, b, 2048]
        def custom_forward_kv(kv_compressed):
            kv, _ = self.linear_kv_up_proj(self.kv_layernorm(kv_compressed))
            return kv, _
        if self.config.q_rms_recompute == True:
            kv, _ = tensor_parallel.checkpoint(custom_forward_kv, False, kv_compressed)
        else:
            kv , _ = custom_forward_kv(kv_compressed)
        #kv, _ = self.linear_kv_up_proj(self.kv_layernorm(kv_compressed))
