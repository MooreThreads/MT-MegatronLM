...
class TransformerConfig(ModelParallelConfig):
    ...
    mtp_coeff: float = 1e-4
    multi_token_prediction_depth: int = 2
    ...
    def __post_init__(self):
        ...
        if self.pipeline_model_parallel_size == 1:
            assert self.first_pipeline_num_layers is None and self.last_pipeline_num_layers is None, \
                f"For pipeline_model_parallel_size=1, first/last must be None, but get {self.first_pipeline_num_layers}/{self.last_pipeline_num_layers}"
        ...
        if self.recompute_granularity is not None:
            if self.recompute_granularity not in ['full', 'selective', 'selective_q_rms']:
                raise ValueError(
