...
def _add_moe_args(parser):
    ...
    group.add_argument('--moe-router-use-sigmoid', action='store_true',
                       help='Use sigmoid instead of softmax routing for MoE, used with moe-router-pre-softmax')
    ...
    group.add_argument('--moe-router-num-node-group', type=int, default=None,
                       help='Number of node groups for the router. Default is None, which is ep_size/8 in deepseekV3.')
    group.add_argument('--moe-noaux-gamma', type=float, default=None,
                       help='The gamma value for the noaux loss, which is used to correct the routing score. Default is None, which means no correction.')
...
def core_transformer_config_from_args(args, config_class=None):
    ...
    kw_args['mtp_coeff'] = args.mtp_coeff
    kw_args['multi_token_prediction_depth'] = args.mtp_depth
